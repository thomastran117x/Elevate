...
.NET 10 background application consuming PostgreSQL search-outbox CDC through Kafka and applying event upserts/deletes to Elasticsearch. The host registers search infrastructure, an index bootstrap service, a scoped message processor, and a Kafka consumer.
...
Provision Kafka, Elasticsearch, and the appropriate PostgreSQL outbox connector: [event-outbox-connector.json](../../../../docker/kafka-connect/event-outbox-connector.json). The worker processes documents carried by messages; normal indexing does not require its own database connection. Compose provisions the database/connector path.
...
```powershell
dotnet run --project backend/src/worker/event-indexer/event-indexer.csproj
docker compose up -d event-indexer
...
```powershell
docker build -f backend/src/worker/event-indexer/Dockerfile -t eventxperience-event-indexer:local backend
```
...
`EventIndexerMessageParser` validates CDC messages and identifies upserts/deletes. `EventIndexerMessageProcessor` retries search failures up to three times with exponential delay starting at 500 ms. Malformed messages and exhausted indexing failures go to the DLQ.
...
```powershell
dotnet test backend.tests.Unit/backend.tests.Unit.csproj --filter "FullyQualifiedName~EventIndexing"
