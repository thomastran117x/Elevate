...
.NET 10 background application consuming PostgreSQL search-outbox CDC through Kafka and applying club-post upserts/deletes to Elasticsearch. The host registers search infrastructure, an index bootstrap service, a scoped message processor, and a Kafka consumer.
...
Provision Kafka, Elasticsearch, and the appropriate PostgreSQL outbox connector: [clubpost-outbox-connector.json](../../../../docker/kafka-connect/clubpost-outbox-connector.json). The worker processes documents carried by messages; normal indexing does not require its own database connection. Compose provisions the database/connector path.
...
```powershell
dotnet run --project backend/src/worker/clubpost-indexer/clubpost-indexer.csproj
docker compose up -d clubpost-indexer
docker compose logs -f clubpost-indexer
```

For a standalone image build, also from the root:

```powershell
docker build -f backend/src/worker/clubpost-indexer/Dockerfile -t eventxperience-clubpost-indexer:local backend
```
...
`ClubPostIndexerMessageParser` validates CDC messages and identifies upserts/deletes. `ClubPostIndexerMessageProcessor` retries search failures up to three times with exponential delay starting at 500 ms. Malformed messages and exhausted indexing failures go to the DLQ.
...
```powershell
dotnet test backend.tests.Unit/backend.tests.Unit.csproj --filter "FullyQualifiedName~ClubPostIndexing"
