...
namespace backend.main.application.environment
{
    public static class EnvironmentSetting
    {
        ...
        static EnvironmentSetting()
        {
            ...
            _emailTopic = GetOrDefault(
                ["EMAIL_TOPIC"],
                "eventxperience-email"
            );
            _emailGroupId = GetOrDefault(
                ["EMAIL_GROUP_ID"],
                "email-worker"
            );
            _emailDlqTopic = GetOrDefault(
                ["EMAIL_DLQ_TOPIC"],
                "eventxperience-email-dlq"
            );
            _smsTopic = GetOrDefault(
                ["SMS_TOPIC"],
                "eventxperience-sms"
            );
            _smsGroupId = GetOrDefault(
                ["SMS_GROUP_ID"],
                "sms-worker"
            );
            _smsDlqTopic = GetOrDefault(
                ["SMS_DLQ_TOPIC"],
                "eventxperience-sms-dlq"
            );
