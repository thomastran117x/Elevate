...
import { PostCommentReaction } from '../../models/club-post.types';
import type {
  PostCommentNode,
  CommentNodeAction,
...
@Component({
  selector: 'app-post-comment-node',
  standalone: true,
  imports: [FormsModule],
  templateUrl: './post-comment-node.component.html',
})
export class PostCommentNodeComponent {
  @Input({ required: true }) node!: PostCommentNode;
  @Input() currentUser: User | null = null;
  @Input() depth = 0;
  @Output() action = new EventEmitter<CommentNodeAction>();
  ...
  get authorName(): string {
    if (this.node.isDeleted) return 'Deleted comment';
  ...
  react(reaction: PostCommentReaction): void {
